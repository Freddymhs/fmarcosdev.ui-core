...
const PILL_STYLES = `
  ...
  .pill__title--primary { opacity: 1; }
  .pill__title--muted { opacity: 1; }
  .pill__item {
    display: flex;
    gap: 4px;
  ...
  @media (max-width: 399px) {
    ...
    .pill__title { display: none; }
  }
  @media (min-width: 400px) and (max-width: 599px) {
    .pill__icon { display: flex; width: 100%; justify-content: center; }
    .pill__title { display: none; }
  }
`;
...
export const DailyFocusPill: React.FC<DailyFocusPillProps> = ({
  ...
  const nextProjectText = nextProject;
  const hasNext = Boolean(nextStudyText || nextProjectText);

  return (
    <>
      <style precedence="default">{PILL_STYLES}</style>
